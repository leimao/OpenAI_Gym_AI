...
class Actor():
    ...
    def Policy_FC_Setup(self):
        ...
        with tf.variable_scope('actor'):
            ...
            self.tf_Q_value = tf.placeholder(tf.float32, None, name = 'Q_value')
            ...
            self.loss = (-1) * tf.log(self.action_probs[0, self.tf_action]) * (self.tf_Q_value)
            
            self.optimizer = tf.train.AdamOptimizer(LEARNING_RATE_ACTOR).minimize(self.loss)

    def Policy_FC_Train(self, observation, action, Q_value):

        observation = observation[np.newaxis, :]

        # Start gradient descent
        _, train_loss = self.sess.run([self.optimizer, self.loss], feed_dict = {self.tf_observation: observation, self.tf_action: action, self.tf_Q_value: Q_value})
...
class Critic():
    ...
    def Value_FC_Train(self, observation, action, reward, done, observation_next, action_next):
